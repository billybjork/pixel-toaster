    ...
    try:
        ...
        if output and output.splitlines():
            ...
            if "version" in first_line.lower():
                ...
                logging.warning(f"Could not parse version from first line: '{first_line}'. Returning full output.")
                return output.strip()
        else:
            return "Could not determine version (no output)."
    except FileNotFoundError:
        # This shouldn't happen if get_ffmpeg_executable worked, but safeguard anyway
        logging.warning(f"ffmpeg executable '{ffmpeg_exe}' not found during version check.")
        return "Unknown (executable not found)"
    except subprocess.TimeoutExpired:
        logging.warning("ffmpeg -version command timed out.")
        return "Unknown (timeout)"
    except Exception as e:
        logging.warning(f"Could not get ffmpeg version using '{ffmpeg_exe}': {e}")
